...
const NAV = [
  ...
  { to: "/dispatch", label: "Dispatch", icon: GitMerge },
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const hydrateDiscovery = useHarness((s) => s.hydrateDiscovery);
    ...
    <div className="weave-bg min-h-dvh">
      ...
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-56 flex-col border-r border-border bg-background/90 px-4 py-6 backdrop-blur-sm lg:flex">
        ...
        <p className="px-2 text-[11px] leading-relaxed text-muted-foreground">
          Default deny. Cloud never sees local-only data.
        </p>
      </aside>
      ...
      <main
        id="main"
        className="px-4 pt-6 pb-28 lg:ml-56 lg:px-10 lg:pt-10 lg:pb-16"
      >
        {children}
      </main>
